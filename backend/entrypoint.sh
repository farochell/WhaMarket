...
# Composer - Install dependencies if vendor directory doesn't exist or is empty
if [ -f "composer.json" ]; then
    if [ ! -d "vendor" ] || [ -z "$(ls -A vendor 2>/dev/null)" ]; then
        echo "📦 Installing Composer dependencies..."
        COMPOSER_ALLOW_SUPERUSER=1 composer install --no-interaction --optimize-autoloader --no-scripts
        echo "🔧 Running Symfony auto-scripts..."
        COMPOSER_ALLOW_SUPERUSER=1 composer run-script --no-interaction auto-scripts
    else
        echo "✅ Composer dependencies already installed"
    fi
fi

# Créer répertoires var avec les bonnes permissions
...
if [ -f "/app/Caddyfile" ]; then
    ...
    # Use SERVER_NAME environment variable (default :80 if not set)
    exec frankenphp php-server --listen ${SERVER_NAME:-:80} --root /app/public
fi
